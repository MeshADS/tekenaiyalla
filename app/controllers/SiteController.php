...
class SiteController extends BaseController {
	...
	*/
	...
	public function __construct(){
		...
		$this->viewdata["searchIn"] = URL::to('');
		$this->viewdata["searchVal"] = Input::get('search', null);
		if(Sentry::check()){
			$user = Sentry::getUser();
	...
	*/
	...
	/**
	* Transforms given date for filter
	* @return Array
	*/
	public function filterDates($year, $month = null, $day = null)
	{
		// Check if month is null
		if (is_null($month)) {
			// Set time/time2
			$time = $year."-01-01 00:00:00";
			$time2 = $year."-12-01 23:59:59";
		} 
		elseif (is_null($day)) {
			// Set time
			...
			// Set time 2
			$time2 = $year."-".$month."-".$lastDay." 23:59:59";
		}
		else{
			// Set time/time2
			$time = $year."-".$month."-".$day." 00:00:00";
			$time2 = $year."-".$month."-".$day." 23:59:59";
		}
